...
namespace structure
{
    ...
    int init(std::string name)
    {
        ...
        {
            std::string mod_path = path_str + cJSON_GetStringValue(cJSON_GetArrayItem(mods, i));
            ...
            HMODULE dll = LoadLibrary(dll_path_wstr.c_str());
            if (dll == NULL)
            {
            ...
            for (int j = 0; j < names_size; j++)
            {
                ...
                if (structure == NULL)
                {
                    glog::log("error", "Failed to get structure", "structure");
                    continue;
                }
